...
## ISS-202 (2026-09-09) — ستّ صورٍ تعدم `required-ci` على `main`: `litellm==1.50.0` لم يعد موجوداً — ✅ مُغلقةٌ بانتظار buildx

**العَرَضُ بما هو قياسٌ لا شكوى:** على `main` عند `4e14758` (تشغيل `34315502753`) **نفسُ**
الستّ حمراء — `image: auditor-service` · `memory-agent` · `observability-service` ·
`planning-agent` · `reasoning-agent` · `user-service` — ومعها `guardrails` و`required-ci`.
أي: علامةُ الصحّ مستحيلةٌ على **كلّ** فرعٍ في المستودع، لا على فرعٍ بعينه؛ و`required-ci`
يحتاج `images-build` (`ci.yml:1276`) فليس خياراً أن «نمرّره».

**التشخيصُ ببردٍ لا برأي:** التعليقُ الوحيد المتاح من `buildx` هو
`failed to solve: process "/bin/sh -c pip install --dry-run…-r requirements.txt" … exit code 1`؛
وأُعيد إنتاجُ السطر محلّياً بمحلّل الصورة نفسِه (`pip 25.0.1 --ignore-installed`) فإذا الخطأُ
`No matching distribution found for litellm==1.50.0` — **النسخةُ سُحِبت من PyPI** (الفهرسُ
1.27.1.x ← 1.53.1). لا علاقةَ لها بكودٍ ولا بسلسلةِ نماذج: ملفّاتُ اعتمادٍ قديمة تحمل مكبحاً
ميّتاً، وهي الملفاتُ الستّة التي وصفها D-205 بأنّها «غير قابلةٍ للحلّ».

**الحلّ = علاجُ D-205 لا أطولُ منه:** حُذِف سطرُ `litellm==1.50.0` مع تعليق الازدواج من
الملفات الستّة (وتُرك `openai==1.55.0` كما هو: رفعُه هجرةُ SDK لا تصليحُ اعتماد). لم يُستبدَل
برقمٍ «يبدو متوافقاً» (1.53.1) لأنّ ذلك يُبقي مكبحاً لا يستورده أحدٌ في خمس خدمات — وهو
بالضبط ما قاله D-205: «الحلُّ إزالةُ التناقض لا مطاردةُ نسخةٍ قديمة».

**البرهانُ المحلّيّ (وبُرْهانُه على ما لا يَدّعي):** محلّلُ الصورة يقبل الملفات الستّة
جميعاً بلا ERROR · `check_supply_chain` ✅ و«دَين التثبيت لم يتّسع: 160» ✅ (لم يزد مجرَّدُ
حذفِ سطرٍ مُثبَّتٍ شيئاً) · `check_legacy_invariants` ✅ 349 تحقّقاً · ولا اختبارَ ولا بوّابةَ
أخرى تَذكُرت `litellm` (grep على `tests/` و`scripts/`: صفر). **⛔ ما لا يُدَّعى:** لم تُبنَ
صورةٌ هنا — لا `docker` في هذه البيئة؛ والفحصُ المكافئ الوحيد هو buildx في `images-build`،
كما قرّر D-205 (2). العهدةُ على تشغيل CIّ التالي.

---

## ISS-200 (2026-09-09) — كلُّ سؤالٍ كان يعود باعتذارٍ واحد: الصدارةُ مُتحقَّقٌ منها وبلا endpoint، والعميلُ لا يدور — ✅ مُغلقة بـD-288

**العَرَض:** كلُّ ردٍّ طالبيٍّ واحدٌ حرفيّاً، مهما كان السؤال، وفي ثلاث ساقٍ مستقلّة:

```
«عذراً، تعذّر الوصول إلى نموذج الذكاء الاصطناعي حالياً. يرجى المحاولة لاحقاً.»
```

مسبارُ الساق المباشرة (`scripts/e2e_orchestrator_live.py`، «اشرح قانون نيوتن الثاني بإيجاز مع مثال عددي»):

```
[2] DIRECT orchestrator stream: ok=False deltas=0 terminal=assistant_final
```

`deltas=0` يعني أنّ البثَّ لم يبدأ أصلاً — فلا هو بطءُ نموذجٍ ولا قطعُ شبكةٍ في المنتصف.

**⚠️ والتشخيص الأوّل كان ناقصاً ويُسجَّل كذلك:** قيل إنّ المفتاح أُنفِد (402). السجلّ يقول
غير ذلك: `openai.APIConnectionError` في `services/llm/client.py` — أي **لا endpoint للنموذج**،
لا رصيدٌ فارغ. والخلط بينهما هو ما يؤخّر الإصلاح: الأول يُداوَى بالدوران، والثاني بالمفاتيح.

**السبب الجذري — ثلاث طبائق، وكلُّها صالحةٌ لتُعمِيَ الحارس الأخضر:**

| # | الطبقة | لماذا لم يَرَهَا CI |
|---|---|---|
| 1 | PRIMARY في السلسلة القانونية (`openai/gpt-oss-20b:free`) بلا endpoint: `GET /api/v1/models/<id>/endpoints` → `"endpoints": []`. ومثلها `gpt-oss-120b:free` و`nemotron-nano-9b-v2:free`. | `live-e2e.yml` يفرض `OPENROUTER_PRIMARY_MODEL=google/gemma-4-31b-it:free` — **تجاوزٌ اختباريّ يُخفي العطبَ الذي يُفترض أنه يقيسه** |
| 2 | عميلُ الـ orchestrator يرسل كلَّ طلب إلى PRIMARY وحده، وأيُّ 4xx من المزوّد قاتل | لا شيء: الوحدةُ تَسخر من `stream_chat` فلا تمرّ بالدوران |
| 3 | `shared/ai_models/registry.py` يقول «قدرةٌ مُتحقَّقة 2026-07-14» — والقدرةُ صحيحة، والخدمةُ ماتت | البوّابةُ تقيس الإدخال لا الكتالوج؛ ولم يكن أحدٌ يسأل الكتالوجَ الحيّ |

**الحلّ (ببوّاباتٍ لا بتعليقات):** السلسلةُ أُعيد بناؤها من الكتالوج الحيّ في مصدرٍ واحد
`shared/ai_models/model_chain.py` (PRIMARY = `google/gemma-4-31b-it:free`)، والدماغان ينعكسان
حرفيّاً فيها تحت `check_model_chain_parity.py`؛ والعميل صار **يدور** على السلسلة عند
402/404/406/408/409/413/425/429/5xx ويفشل فوراً على 401/403 (مفتاحٌ ميتٌ لا يُداوَى بنموذج آخر)،
بـ`max_retries=0` (الآليةُ هي السلسلة) ومهلةِ أوّل رمز، ويُلغي الدورانَ عند أوّل رمزٍ مصادق عليه
(لا إزدواجٌ في البثّ)، ويَعُدّ `content_chunks==0` فشلاً لا إجابة، ويُبلِّغ `last_model` و
`health_snapshot()["chain"]`. المسارُ الطالبي يُميِّز `provider_down` عن «لا سياق» ولا يعيد
محاولةَ DSPy على مزوّدٍ ميت. و`OPENROUTER_BASE_URL` صار **حقلَ ضبط** في `Settings` الدماغين
يقرؤه عميلُ الـ orchestrator ووكيلا `simple_client` وعقدةُ DSPy — لا `os.getenv` مُكرَّراً في خمسة
ملفات، لأنّ حارسَ الحرفيات (D-270 L5) يُحمِّر CI على كلِّ اسمٍ بلا موطن، وقد حمّرَنا في أول تشغيلٍ
لهذا الفرع. تشخيصُ ISS-200 استحال بلا بوابةٍ تُوجَّه إلى مزدوج — والفرقُ أن المقبضَ الآن في موطنه.

**البرهان (حيٌّ، لا ساخر):** بعد الإصلاح، على الساق الثلاث جميعها — `POST /api/chat/messages`
مباشراً، وWS عبر المونوليث :8000، وWS عبر وكيل الواجهة :5000:

```
ok=True deltas=10 chars=245 ttft=0.04 terminal=assistant_final
RESULT: direct=True monolith=True frontend=True
```

ولإثبات الدوران **لا التوصيةَ به** فقط، أُقيلِعت الخدمةُ بـ`OPENROUTER_PRIMARY_MODEL=openai/gpt-oss-20b:free`
(نموذجٌ ميتٌ عمداً):

```
WARNING:ai-client:AI stream: model=openai/gpt-oss-20b:free failed (NotFoundError(404) ... 'No endpoints found for model ...') — rotating
INFO:ai-client:AI stream served by model=google/gemma-4-26b-a4b-it:free chunks=9
```

وسجّل المسبارُ البثَّ كاملاً (249 حرفاً، `deltas=10`) — أي أنّ الطالب لم يعد يرى الاعتذار.

**فرضٌ دائم:** `scripts/verify_model_registry_live.py` (`make model-check`) يسأل الكتالوجَ الحيّ
عن السلسلة **كما يقرؤها الدماغان** (تجاوزُ المشغِّل ثم الذيل ثم `OPENROUTER_EXTRA_MODELS`)، مُدرَجٌ
في `live-e2e.yml` قبل إقلاع الخادم، وfail-closed عند تعذّر الشبكة (خروج 2 لا أخضر كاذب).

**شرط الإغلاق (كُتِب قبل النتيجة):** الساقُ المباشر يُعيد دلالةَ `chat` كاملة · الموتُ في البيئة الحيّة
يُعلَن ولا يُستَّر بتجاوزٍ اختباريّ · كلُّ قناةٍ تمرّ بالمزوّد تقرأ `OPENROUTER_BASE_URL` · وسجلُّ
`shared/ai_models/registry.py` لا يُخدَم بالادّعاء: من دخل السلسلة دخل السجلَّ بدليلٍ مؤرَّخ.

**إلحاقٌ من أول تشغيلٍ CIّ (2026-09-09):** البوّابةُ الجديدة كادت تُعلِّم الفريق تجاهلها — كانت
تُفشِل `live-e2e` عند 429/انقطاعٍ من مشغِّل GitHub، أي تُعاقب على عَمىً لا على عطب. صُحِّح الحدُّ
نفسُه: موتٌ مقطوعٌ به (200 مع `"endpoints": []` · أو 404) ⇒ إفشال؛ لم نرَه ⇒ تحذيرٌ صريحٌ
و`::warning::` وبرمزٍ نظيف؛ `--strict` لمن يرفض التشغيل بلا رؤية؛ و401/403 على `/auth/key` خطأٌ
قطعيّ لأنّ مفتاحاً مرفوضاً يجعل كلَّ أخضرَ كذباً. — **وشقُّ 403 من هذه الجملة نُقضَ في ثاني
تشغيلٍ (2026-09-09):** 403 على `/auth/key` يصل من جدارٍ أماميّ لا من OpenRouter، فصار تحذيراً
يرتقي خطأً تحت `--strict`؛ اليقينُ بقي 401 وحده (التفصيل في D-288، إلحاق 2026-09-09 بند 3).
وفي **الثالث** تبيّن أنّ المُحمِّرَ لم يكن المفتاحَ قطّ: المسبارُ كان يقرأ **أيّ** 404 —
و**أيّ** 200 بجسدٍ لا `data` فيه — «نموذجاً غير موجود»، حكماً إفشاليّاً على رمزِ حالةٍ من جدارٍ
لا يعرف هذا المسار. صار اليقينُ مشروطاً بإجابةٍ مقروءةٍ من البوّابة (JSON فيه `error`)، وصارت
الخطوةُ تطبعُ حكمَها في `::error::` لأنّ سجلَّ المشغِّل لا يُقرأ من هنا (تفصيلٌ في D-288 بند 4).
وبفضل تلك الانوتيشن عُرفَ السببُ في **تشغيلٍ واحد**: المسارُ كان `models/google%2F…%3Afree/endpoints`
فتردُّ الحافّةُ بـ404 موسومٍ لكلّ معرّف، فيُقرأ «الكتالوجُ لا يعرف نموذجَنا» ستَّ مرّات. صار
البناءُ `safe='/:'`، وأُضيف سطرُ `sought:`، وصار **«كلُّ السلسلة غائبة بلا إجابةٍ موسومة» عمًى
مُعلَناً لا حكماً** (D-288 بند 5) — ولا تُبنى خُضرةُ منتجٍ على سؤالٍ لم نطرحه.

---

## ISS-201 (2026-09-09) — حارسُ النزاهة كان يمزّق الإجابة الصحيحة: `https://…` تصير `://./` — ✅ مُغلقة بـD-289

**العَرَض:** بعد إصلاح ISS-200 مباشرةً، وصلت الإجابةُ **مقطوعة الأوصال** عبر ساقَي المونوليث
والواجهة، بينما هي سليمةٌ في الساق المباشرة:

```
$$F = m \cdot$$ …  ‎2 ././‏ … python3.12 → 3.12
```

**التشخيص:** استُبعِل واحدًا واحدًا `arabic_stream_guard` و`local_graph_sanitizers._FOREIGN_REPLACEMENTS`
و`text_streaming._strip_retrieval_tags`/`_sanitize_text_for_user` و`response_sanitizer` — كلّها بريئةٌ
على نفس الحرف. المُذنِب: `StreamIntegrityFilter._strip_latin_garbage` في `app/services/skills/`
`content_integrity_skill.py`: كلّ لاتينيٍّ طويلٍ غارباجٌ عنده، فتُشطَب الروابط والمسارات والمُعرِّفات
والمصطلحات العلمية. **وفخٌّ منهجيّ سُجِّل:** العيّنةُ القصيرة (< 200 حرف) لا تُفعِّل `arabic_mode`
(`_DECISION_WINDOW=200`)، فتبدو الحُرّاسُ خاملةً في الاختبار اليدويّ وتعود وحشاً في الردّ الطويل.

**الحلّ لا التليين:** استثناءٌ بنيوي `_is_structural_token` (رابطٌ/مسار/مُعرِّف/شيفرةٌ مُضمَّنة لا
تُحذف) + توسعةُ `_TECH_ALLOWLIST` بمفردات SI والفيزياء والكيمياء وأسماء الملفات — لا تخفيفُ العتبة
ولا إطالةُ النافذة، فكلُّ ذلك يُعيد تسريبَ التفكير إلى الطالب. وبندٌ في دستور الطبقة مع إصداره:
`CONTENT_INTEGRITY_DOCTRINE_VERSION` من `1.0.0` إلى `1.1.0` (يقرؤه `manifest_registry.py`، فالبندُ
والرقمُ لا ينفصلان).

**البرهان:** جوابٌ طالبيٌّ واقعي فيه `Newton` و`volt` و`joule` و`https://example.com/physics` و`python3.12`
→ `INTACT: True`؛ واللاتينيُّ المترهّل `experiences_random Eingaben Sweg exitos` → **ما زال يُشطَب**.
وساقا المونوليث والواجهة تُعيدان الآن نفس الـ 245 حرفاً حرفيّاً.

**شرط الإغلاق (كُتِب قبل النتيجة):** الحارسُ أستاذٌ لا رقابة: يحمي البنيةَ ويحذف الهذيان · وكلُّ توسعةِ
قائمةٍ بيضاء تُعلَن بنداً في دستور الطبقة لا صمتاً في الكود · والعيّنةُ في أيِّ تشخيصٍ يدويٍّ تتجاوز
نافذةَ القرار، وإلّا كذبت البراهين.
