...
async def test_customer_chat_persists_conversation_and_messages(
    ...
    """يتحقق من تمرير أحداث المحادثة الأساسية عبر قناة العميل دون تعليق."""

    async def _stream_events() -> AsyncGenerator[dict[str, object], None]:
        yield {"type": "conversation_init", "payload": {"conversation_id": 101, "title": "Vectors"}}
        yield {"type": "delta", "payload": {"content": "Persisted assistant answer"}}
        yield {"type": "complete", "payload": {"status": "done"}}
    ...
    assert any(event.get("type") in {"delta", "assistant_delta"} for event in events)
...
async def test_customer_chat_history_endpoint_reads_persisted_websocket_messages(
    ...
    """يتحقق من أن واجهة التاريخ تعرض الرسائل المخزنة لمالك المحادثة."""
    ...
    try:
        ...
        conversation = CustomerConversation(title="Vectors", user_id=user.id)
        db_session.add(conversation)
        await db_session.flush()
        db_session.add(
            CustomerMessage(
                conversation_id=conversation.id,
                role=MessageRole.USER,
                content="اشرح تمرين المتجهات في البكالوريا",
            )
        )
        db_session.add(
            CustomerMessage(
                ...
                content="History visible answer",
            )
        )
        await db_session.commit()
...
async def test_customer_chat_dispatch_receives_mission_metadata_and_conversation_id(
    ...
    def mock_chat_with_agent(**kwargs: object) -> AsyncGenerator[dict[str, object], None]:
        captured["user_id"] = kwargs.get("user_id")
        captured["conversation_id"] = kwargs.get("conversation_id")
        context = kwargs.get("context")
        captured["metadata"] = context.get("metadata") if isinstance(context, dict) else None
    ...
    try:
        with patch(
            "app.api.routers.customer_chat.orchestrator_client.chat_with_agent",
            side_effect=mock_chat_with_agent,
        ):
...
async def test_customer_websocket_persists_fallback_message_on_stream_failure(
    ...
    """يتحقق من إرجاع رسالة خطأ واضحة عند فشل بث المزوّد."""

    def failing_chat_with_agent(**kwargs: object) -> AsyncGenerator[dict[str, object], None]:
    ...
    try:
        with patch(
            "app.api.routers.customer_chat.orchestrator_client.chat_with_agent",
            side_effect=failing_chat_with_agent,
        ):
    ...
    assert any(event.get("type") in {"error", "assistant_error"} for event in events)
