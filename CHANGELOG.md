...
- **والجذرُ الحقيقيّ لذلك الحمرة كان في المسبار نفسه (وثالثُ تصحيحٍ في اليوم).** كان
  `probe_model` يقرأ **أيّ** 404 — و**أيّ** 200 بجسدٍ لا `data` فيه — «نموذجاً غير موجود»،
  وهو حكمٌ يُفشِل؛ غير أنّ جداراً أماميّاً في مسار مشغِّلات GitHub يُرجِع 404 بـHTMLّ لمن
  لا يعرف المسار، فتحمرّ الرحلةُ على رمزِ حالةٍ لم يُجِب عن سؤالها. اليقينُ بالموت صار
  مشروطاً **بإجابةٍ مقروءةٍ من البوّابة** (JSON فيه `error`)، وكلُّ ما عداه عمًى ⇒ تحذيرٌ
  وخروجٌ نظيف؛ وحملت الخطوةُ نفسها الحكمَ في `::error::` لأنّ سجلَّ المشغِّل لا يُقرأ من
  هنا و«exit code 1» وحده لا يُشخَّص. 17 اختباراً (كانت 13) تُميِّز الجوابَ من الارتطام.

- **والسببُ النهائيُّ لحُمرة المسبار: لم يكن الكتالوجُ، بل سؤالٌ مُرمَّزٌ خطأً.** الانوتيشنُ
  الذي أضفناه هو الذي قال ذلك في تشغيلٍ واحد: `in_catalog=False` للنماذج الستّة جميعاً، لأنّ
  المسارَ كان يُبنى بـ`quote(model, safe='')` فتصير `/` و`:` إلى `%2F`/`%3A` وتردُّ الحافّةُ
  بـ404 موسومٍ (`error`) لا يعرف ذلك الشكل — أي أنّ شرطَ «إجابةٍ مقروءة» الذي وضعناه ليمنع
  اليقينَ الزائفَ مرَّ منه يقينٌ زائفٌ آخر. الصواب: `safe='/:'` كما في وثائق المزوّد، وسطرُ
  `sought:   GET …` في رأس الجدول، وقاعدةٌ أن **كلَّ السلسلة «غير موجودة» بلا إجابةٍ موسومةٍ
  واحدة تُعلَن عمًى لا حكماً** (وتُرفَض تحت `--strict`). 20 اختباراً (كانت 17) تسألُ السؤالَ
  نفسَه: لا ترميزَ في المعرّف، والعمى الشاملُ خروجٌ نظيف، وإجابةٌ موسومةٌ واحدة تُعيد الحكمُ قاطعاً.
